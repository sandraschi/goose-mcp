...
# PyInstaller spec — produce goose-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller goose-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/goose_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/goose_mcp", "goose_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
